...
export class IndexRanges {
     ...
     * Single pass, O(total).
     */
    static fromPredicate(total: number, pred: (i: number) => boolean): IndexRanges {
        const ranges: number[] = [];
        let rangeStart = -1;
    ...
    forEachRun(fn: (start: number, count: number) => void): void {
        const { data } = this;
        let r = 0;
        while (r < data.length) {
            if (data[r] & SINGLE_BIT) {
